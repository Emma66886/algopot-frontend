...
const navigate = useNavigate()
  const registerUser = ()=>{
    ...
    axios({
      ...
      data: {
        ...
        console.log(e);
        setRequestload(false)
        setError((typeof e?.response.data != Object && e?.response?.data?.message) || e?.message || "An error occurred.")
      })
  }
  ...
  return (
    ...
    <Input value={userName} {...commonInputStyle} onChange={(e)=>setUsername(e.target.value)} bg='#fff' w='80%' h='50px' fontWeight='500' fontFamily='Poppins' fontSize='18px' lineHeight='27px'  placeholder='Username' borderRadius='4px'/>
    {userNameErr != '' && <Text color="#ff0000" fontSize='14px'>{userNameErr}</Text>}
   
    <Input value={firstname} {...commonInputStyle} onChange={(e)=>setFirstname(e.target.value)} bg='#fff' w='80%' h='50px' fontWeight='500' fontFamily='Poppins' fontSize='18px' lineHeight='27px'  placeholder='First Name' borderRadius='4px'/>
    {firstNameErr != '' && <Text color="#ff0000" fontSize='14px'>{firstNameErr}</Text>}
    
    <Input value={lastName} {...commonInputStyle}  onChange={(e)=>setLastName(e.target.value)} bg='#fff' w='80%' h='50px' fontWeight='500' fontFamily='Poppins' fontSize='18px' lineHeight='27px'  placeholder='Last Name' borderRadius='4px'/>
    {lastNameErr != '' && <Text color="#ff0000" fontSize='14px'>{lastNameErr}</Text>}
    
    <Input value={email} {...commonInputStyle}  onChange={(e)=>setEmail(e.target.value)} bg='#fff' w='80%' h='50px' fontWeight='500' fontFamily='Poppins' fontSize='18px' lineHeight='27px'  type='email' placeholder='Email Address' borderRadius='4px'/>
    {emailErr != '' && <Text color="#ff0000" fontSize='14px'>{emailErr}</Text>}
    
    <Input borderColor={error !='' && '#ff0000'} {...commonInputStyle}   value={password} onChange={(e)=>setPass(e.target.value)} bg='#fff' w='80%' h='50px' fontWeight='500' fontFamily='Poppins' fontSize='18px' lineHeight='27px'  type='password' placeholder='Password' borderRadius='4px'/>
    <Input borderColor={error !='' && '#ff0000'} {...commonInputStyle}  value={confirmPass} onChange={(e)=>setConfirmPass(e.target.value)} bg='#fff' w='80%' h='50px' fontWeight='500' fontFamily='Poppins' fontSize='18px' lineHeight='27px'  type='password' placeholder='Confirm Password' borderRadius='4px'/>
    </InputGroup>
